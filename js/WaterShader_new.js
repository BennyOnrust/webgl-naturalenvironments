...
THREE.ShaderLib['waterFar'] = {
	...
	fragmentShader: [
		...
		'{',
		'	vec4 noise = getNoise( worldPosition.xz );',
		'	vec3 surfaceNormal = normalize( noise.xzy * vec3( .915, 1.0, .915 ) );',
...
THREE.Water = function ( renderer, camera, scene, options ) {
	...
	this.distortionScale = optionalParameter( options.distortionScale, 20.0 );
        this.near = optionalParameter( options.near, true );
	...
	this.tempTexture = new THREE.WebGLRenderTarget( width, height );
        ...
        if(this.near){
            var mirrorShader = THREE.ShaderLib[ 'waterNear' ];
        }
        else{
            var mirrorShader = THREE.ShaderLib[ 'waterFar' ];
        }
